...
const repeated=rows.filter((r,i)=>i>=2&&JSON.stringify(levels[i].solution)===JSON.stringify(levels[i-1].solution));
const report={version:'3.0.0-rc.2',logicCertified:levels.length===100&&issues.length===0&&repeated.length===0,levelDataSha256:crypto.createHash('sha256').update(fs.readFileSync(new URL('../dist/js/levels.js',import.meta.url))).digest('hex'),certified:rows.filter(r=>!issues.includes(r)).length,blockers:issues.map(r=>({level:r.level,solutions:r.solutions,reason:'Error de validación'})),consecutiveRepeatedSolutions:repeated.map(r=>r.level),rows};
...
console.log(JSON.stringify({certified:report.certified,blockers:report.blockers,consecutiveRepeatedSolutions:report.consecutiveRepeatedSolutions,geometricDuplicates:rows.filter(r=>r.duplicateOf).map(r=>[r.level,r.duplicateOf]),ratingBySize:[4,5,6,7].map(n=>({size:n,min:Math.min(...rows.filter(r=>r.size===n).map(r=>r.rating)),max:Math.max(...rows.filter(r=>r.size===n).map(r=>r.rating))}))},null,2));
