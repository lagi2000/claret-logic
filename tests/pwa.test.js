...
function runtime(){
 const events={},stores=new Map(),deleted=[],scope='https://colegio.example/juegos/claret/',foreign='claret-logic-%2Fotra-app%2F-old';stores.set(foreign,new Map());
 ...
 vm.runInNewContext(source,{URL,caches,fetch:async()=>{throw Error('offline');},self:{registration:{scope},clients:{claim:async()=>{}},addEventListener:(name,handler)=>events[name]=handler}});
 ...
 return {events,stores,deleted,foreign,lifecycle,request};
}
