...
function runtime(){
 const events={},stores=new Map(),deleted=[],lifecycleState={skipWaiting:false},scope='https://colegio.example/juegos/claret/',foreign='claret-logic-%2Fotra-app%2F-old';stores.set(foreign,new Map());
 ...
 const caches={keys:async()=>[...stores.keys()],delete:async k=>{deleted.push(k);return stores.delete(k);},open:async k=>{
   ...
   return {addAll:async files=>{for(const f of files)m.set(f,{url:f,body:'cached '+f});},match:async request=>m.get(typeof request==='string'?request:request.url)};
 }};
 vm.runInNewContext(source,{URL,caches,fetch:async()=>{throw Error('offline');},self:{registration:{scope},clients:{claim:async()=>{}},skipWaiting:async()=>{lifecycleState.skipWaiting=true;},addEventListener:(name,handler)=>events[name]=handler}});
 ...
 return {events,stores,deleted,foreign,lifecycleState,lifecycle,request};
}
